...
//                                TipHelper.renderGenericTip((float) InputHelper.mX + 20.0F * Settings.scale, (float) InputHelper.mY, hovered.name, "下回合开始阶段将生产" + hovered.cardsToPreview.name);
                                ...
                                TipHelper.renderGenericTip((float) InputHelper.mX + 20.0F * Settings.scale, (float) InputHelper.mY, hovered.name, (hovered.rawDescription).replace("momod:", "").replace("!M!", String.format("%d", hovered.magicNumber)));
                        }
                    }
                }
